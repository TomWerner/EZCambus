...
public class StopFinderFragment extends Fragment
{
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container, Bundle savedInstanceState)
    {
        // if (container == null)
        // return null;
        View rootView = inflater.inflate(R.layout.stop_finder_fragment, container, false);
    ...
    private void adjustVisibleStops()
    {
        ...
        Float[] distances = closestStops.keySet().toArray(new Float[closestStops.keySet().size()]);
        Arrays.sort(distances);

        setStartMarker(startMarker.getPosition(), startMarker.getTitle());
        MarkerOptions mark;

        Float[] keys = closestStops.keySet().toArray(new Float[closestStops.size()]);
        Arrays.sort(keys);
        ...
        for (int i = 0; i < keys.length; i++)
        {
            ...
            if (distance <= maxDistance)
            {
                ...
                mark = new MarkerOptions().position(stop.getPosition()).title(stop.toString());
        ...
        StopFinderManager nsm = new StopFinderManager(getActivity().getApplicationContext(), getActivity(), listItems);
        ...
        stopListView.setLayoutParams(new TableLayout.LayoutParams(LayoutParams.WRAP_CONTENT, height * 3));
    }

    private void initializeSearchAutocomplete(View rootView)
    {
        addressSearch = (AutoCompleteTextView) rootView.findViewById(R.id.startAddressSearch);
    ...
    protected boolean addressSearchTextSubmit(TextView v, int actionId, KeyEvent event)
    {
        ...
        if (query != null)
        {
            ...
            if (result != null)
            {
                ...
                setStartMarker(new LatLng(result.getLatitude(), result.getLongitude()), query);
                final LatLng point = new LatLng(result.getLatitude(), result.getLongitude());
    ...
    private void setStartMarker(LatLng point, String name)
    {
        ...
        MarkerOptions mark = new MarkerOptions().position(new LatLng(point.latitude, point.longitude)).title(name).draggable(true)
    ...
    private LatLngBounds createBounds()
    {
        ...
        System.out.println(new LatLng(minLat, minLng).toString() + new LatLng(maxLat, maxLng).toString());
